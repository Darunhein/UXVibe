...
public class DeleteParticipantServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (testName == null || testName.trim().isEmpty() || participantName == null || participantName.trim().isEmpty()) {
      resp.sendRedirect(
          req.getHeader("Referer") == null ? req.getContextPath() + "/participants" : req.getHeader("Referer"));
      return;
    }

    ParticipantStore.getInstance().deleteParticipant(account.getEmail(), testName, participantName);

    resp.sendRedirect(req.getContextPath() + "/participants?testName="
        + java.net.URLEncoder.encode(testName, java.nio.charset.StandardCharsets.UTF_8));
  }
}
